 ...
 * in the token, set {@see setMinTokenLength MinTokenLength} and {@see setMaxTokenLength MaxTokenLength}.
 * To use case-insensitive comparison and generate upper-case-only token, set {@see setCaseSensitive CaseSensitive}
 * to false. Advanced users can try to set {@see setTokenAlphabet TokenAlphabet}, which
 * specifies what characters can appear in tokens.
 *
 * The validation of the token is related with two properties: {@see setTestLimit TestLimit}
 * and {@see setTokenExpiry TokenExpiry}. The former specifies how many times a token can
 * be tested with on the server side, and the latter says when a generated token will expire.
 *
 * To specify the appearance of the generated token image, set {@see setTokenImageTheme TokenImageTheme}
 * to be an integer between 0 and 63. And to adjust the generated image size, set {@see setTokenFontSize TokenFontSize}
 * (you may also set {@see \Prado\Web\UI\WebControls\TWebControl::setWidth Width}, but the scaled image may not look good.)
 * By setting {@see setChangingTokenBackground ChangingTokenBackground} to true, the image background
 ...
 * The {@see \Prado\Web\UI\WebControls\TCaptchaValidator} control can also be used to do validation, which provides
