 ...
 *
 * @author Qiang Xue <qiang.xue@gmail.com>
 * @version $Id: CActiveRecord.php 1127 2009-06-13 20:26:35Z qiang.xue $
 * @package System.Testing.Data.ActiveRecord
 * @since 1.0
 */

Prado::using('System.Base.TModel');
Prado::using('System.Testing.Data.*');
Prado::using('System.Testing.Data.ActiveRecord.*');
Prado::using('System.Base.TEvent');
Prado::using('System.Testing.Data.ActiveRecord.TActiveRecordCriteria');

abstract class TActiveRecord extends TModel
{
	const BELONGS_TO='TBelongsToRelation';
	const HAS_ONE='THasOneRelation';
	const HAS_MANY='THasManyRelation';
	const MANY_MANY='TManyManyRelation';
	const STAT='TStatRelation';
...
#			return parent::__isset($name);
	 ...
	 * @since 1.0.5
	 * 
	 * @TODO: implement __call to parent
	 * 
	 */
...
# 		return parent::__call($name,$parameters);*/
	...
	public function getRelated($name,$refresh=false,$params=array())
	{
		...
		if(!isset($md->relations[$name]))
			throw new TDbException(get_class($this).' does not have relation "'.$name.'".');

		Prado::trace('lazy loading '.get_class($this).'.'.$name,'System.Testing.Data.ActiveRecord.TActiveRecord');
		$relation=$md->relations[$name];
		if($this->getIsNewRecord() && ($relation instanceof THasOneRelation || $relation instanceof THasManyRelation))
			return $relation instanceof THasOneRelation ? null : array();
		...
		$finder=new TActiveFinder($this,$r);
		$finder->lazyFind($this);

		if(!isset($this->_related[$name]))
		{
			if($relation instanceof THasManyRelation)
				$this->_related[$name]=array();
			else if($relation instanceof TStatRelation)
	...
	public static function model($className=__CLASS__)
	{
		...
		else
		{
			...
			$model->_md=new TActiveRecordMetaData($model);
			return $model;
		}
	}
	...
	/**
	 * Returns connection id (must be overloaded by child classes)
	 * @return string Id of TDbConnection module
	 */
	...
	{
		$this->onBeforeSave(new TEvent($this));
		return true;
	}
	...
	{
		$this->onAfterSave(new TEvent($this));
	}
	...
	{
		$this->onBeforeDelete(new TEvent($this));
		return true;
	}
	...
	{
		$this->onAfterDelete(new TEvent($this));
	}
	...
	{
		$this->onAfterConstruct(new TEvent($this));
	}
	...
	{
		$this->onAfterFind(new TEvent($this));
	}
	...
	public function insert($attributes=null)
	{
		if(!$this->getIsNewRecord())
			throw new TDbException('The active record cannot be inserted to database because it is not new.');
		if($this->beforeSave())
		{
			Prado::trace(get_class($this).'.insert()','System.Testing.Data.ActiveRecord.TActiveRecord');
	...
	{
		if($this->getIsNewRecord())
			throw new TDbException('The active record cannot be updated because it is new.');
		if($this->beforeSave())
		{
			Prado::trace(get_class($this).'.update()','System.Testing.Data.ActiveRecord.TActiveRecord');
	...
	public function saveAttributes($attributes)
	{
		if(!$this->getIsNewRecord())
		{
			Prado::trace(get_class($this).'.saveAttributes()','System.Testing.Data.ActiveRecord.TActiveRecord');
			$values=array();
	...
	public function delete()
	{
		if(!$this->getIsNewRecord())
		{
			Prado::trace(get_class($this).'.delete()','System.Testing.Data.ActiveRecord.TActiveRecord');
			...
			throw new TDbException('The active record cannot be deleted because it is new.');
	}
	...
	{
		Prado::trace(get_class($this).'.refresh()','System.Testing.Data.ActiveRecord.TActiveRecord');
	...
	{
		Prado::trace(get_class($this).'.find()','System.Testing.Data.ActiveRecord.TActiveRecord');
	...
	{
		Prado::trace(get_class($this).'.findAll()','System.Testing.Data.ActiveRecord.TActiveRecord');
		$criteria=$this->getCommandBuilder()->createCriteria($condition,$params);
	...
	{
		Prado::trace(get_class($this).'.findByPk()','System.Testing.Data.ActiveRecord.TActiveRecord');
	...
	{
		Prado::trace(get_class($this).'.findAllByPk()','System.Testing.Data.ActiveRecord.TActiveRecord');
		$criteria=$this->getCommandBuilder()->createPkCriteria($this->getTableSchema(),$pk,$condition,$params);
		return $this->query($criteria,true);
	}
	...
	{
		Prado::trace(get_class($this).'.findByAttributes()','System.Testing.Data.ActiveRecord.TActiveRecord');
	...
	{
		Prado::trace(get_class($this).'.findAllByAttributes()','System.Testing.Data.ActiveRecord.TActiveRecord');
		$criteria=$this->getCommandBuilder()->createColumnCriteria($this->getTableSchema(),$attributes,$condition,$params);
		return $this->query($criteria,true);
	}
	...
	{
		Prado::trace(get_class($this).'.findBySql()','System.Testing.Data.ActiveRecord.TActiveRecord');
	...
	{
		Prado::trace(get_class($this).'.findAllBySql()','System.Testing.Data.ActiveRecord.TActiveRecord');
		$command=$this->getCommandBuilder()->createSqlCommand($sql,$params);
	...
	{
		Prado::trace(get_class($this).'.count()','System.Testing.Data.ActiveRecord.TActiveRecord');
		$builder=$this->getCommandBuilder();
	...
	{
		Prado::trace(get_class($this).'.countBySql()','System.Testing.Data.ActiveRecord.TActiveRecord');
	...
	{
		Prado::trace(get_class($this).'.exists()','System.Testing.Data.ActiveRecord.TActiveRecord');
		$builder=$this->getCommandBuilder();
		$criteria=$builder->createCriteria($condition,$params);
	...
	public function with()
	{
		if(func_num_args()>0)
		{
			...
			if(is_array($with[0]))  // the parameter is given as an array
				$with=$with[0];
			$finder=new TActiveFinder($this,$with,$this->getDbCriteria(false));
			$this->_c=null;
			return $finder;
		}
		else
	...
	{
		Prado::trace(get_class($this).'.updateByPk()','System.Testing.Data.ActiveRecord.TActiveRecord');
		$builder=$this->getCommandBuilder();
		$table=$this->getTableSchema();
	...
	{
		Prado::trace(get_class($this).'.updateAll()','System.Testing.Data.ActiveRecord.TActiveRecord');
		$builder=$this->getCommandBuilder();
		$criteria=$builder->createCriteria($condition,$params);
		$this->applyScopes($criteria);
	...
	{
		Prado::trace(get_class($this).'.updateCounters()','System.Testing.Data.ActiveRecord.TActiveRecord');
		$builder=$this->getCommandBuilder();
		$criteria=$builder->createCriteria($condition,$params);
		$this->applyScopes($criteria);
	...
	{
		Prado::trace(get_class($this).'.deleteByPk()','System.Testing.Data.ActiveRecord.TActiveRecord');
		$builder=$this->getCommandBuilder();
	...
	{
		Prado::trace(get_class($this).'.deleteAll()','System.Testing.Data.ActiveRecord.TActiveRecord');
		$builder=$this->getCommandBuilder();
		$criteria=$builder->createCriteria($condition,$params);
		$this->applyScopes($criteria);
		$command=$builder->createDeleteCommand($this->getTableSchema(),$criteria);
 ...
 * @since 1.0.4
 */
class TBaseActiveRelation extends TComponent
{
	/**
 ...
 * @since 1.0.4
 */
class TStatRelation extends TBaseActiveRelation
{
	/**
 ...
 * @since 1.0
 */
class TActiveRelation extends TBaseActiveRelation
{
	/**
 ...
 * @since 1.0
 */
class TBelongsToRelation extends TActiveRelation
{
}
 ...
 * @since 1.0
 */
class THasOneRelation extends TActiveRelation
{
}
 ...
 * @since 1.0
 */
class THasManyRelation extends TActiveRelation
{
	/**
 ...
 * @since 1.0
 */
class TManyManyRelation extends THasManyRelation
{
}
...
class TActiveRecordMetaData
{
	...
	public function __construct($model)
	{
		...
		if(($table=$model->getDbConnection()->getSchema()->getTable($tableName))===null)
			throw new TDbException('The table "'.$tableName.'" for active record class "'.get_class($model).'" cannot be found in the database.');
		...
		foreach($model->relations() as $name=>$config)
		{
			if(isset($config[0],$config[1],$config[2]))  // relation class, AR class, FK
				...
				throw new TDbException('Active record "'.get_class($model).'" has an invalid configuration for relation "'.$name.'". It must specify the relation type, the related active record class and the foreign key.');
		}
	}
}
