...
class TDbCommand extends TComponent
{
	...
	public function execute()
	{
		...
		try
		{
			/*if($this->_connection->enableProfiling)
				Yii::beginProfile('system.db.TDbCommand.execute('.$this->getText().')','system.db.TDbCommand.execute');
			*/
			...
			/*if($this->_connection->enableProfiling)
				Yii::endProfile('system.db.TDbCommand.execute('.$this->getText().')','system.db.TDbCommand.execute');
			*/
			return $n;
		}
		catch(Exception $e)
		{
			/*if($this->_connection->enableProfiling)
				Yii::endProfile('system.db.TDbCommand.execute('.$this->getText().')','system.db.TDbCommand.execute');
			 */
			Prado::log('Error in executing SQL: '.$this->getText().$params,TLogger::ERROR,'system.db.TDbCommand');
	...
	private function queryInternal($method,$mode)
	{
		...
		{
			/*if($this->_connection->enableProfiling)
				Yii::beginProfile('system.db.TDbCommand.query('.$this->getText().')','system.db.TDbCommand.query');
			*/
			if($this->_statement instanceof PDOStatement)
				$this->_statement->execute();
			else
