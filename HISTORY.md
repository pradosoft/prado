## Version 4.4.0 - TBA
ENH: Shipped control stylesheets are color-scheme aware. `accordion.css`, `tabpanel.css`, `keyboard.css`, `TSlider/TSlider.css`, `datepicker/default.css`, `colorpicker/default.css` and the ActiveRecord Scaffold `style.css` resolve their colors through `light-dark()` and the `Canvas`/`CanvasText` system colors, so controls follow the `color-scheme` the application declares, including a scheme declared on a single container. An application that declares no `color-scheme` renders exactly as before. Dark values are contrast-audited against WCAG 1.4.3 and 1.4.11. (belisoful)
