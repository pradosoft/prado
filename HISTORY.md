## Version 4.4.0 - TBA
ENH: `TPage::StateCompressionMethod` selects the content coding the page state is compressed with, and `TPageStateFormatter` routes `EnableStateCompression` through `TCompression` instead of calling `gzcompress()`/`gzuncompress()` directly, so `zstd`, `br` and `gzip` are available where the extension is. The default `deflate` is the zlib format the formatter has always written, so a state written by an earlier version still reads. A state that fails to decrypt or to decompress is reported as corrupted rather than being carried forward. (belisoful)
...
BUG: The PHPStan guard extensions narrowed a PRADO virtual property under its lowercase-first spelling only, so `$obj->Title` inside a `canGetProperty('Title')`, `canSetProperty('Title')` or `hasMethod('getTitle')` guard was reported as an undefined property. PHPStan asks for a property with the spelling written in the source, and PRADO writes `Title`. `TComponentCanGetPropertyTypeSpecifyingExtension`, `TComponentCanSetPropertyTypeSpecifyingExtension` and `TComponentHasMethodTypeSpecifyingExtension` register both spellings; the accessor name parsing is extracted as `TComponentHasMethodTypeSpecifyingExtension::propertyNameFromAccessor()`. (belisoful)
ENH: `TComponent::isa()` carries its type narrowing in an `@phpstan-assert-if-true` tag, so PHPStan narrows the subject of an `isa()` guard whatever its type. PHPStan offers a method call to a `MethodTypeSpecifyingExtension` only when the subject has exactly one object class name, so an intersection subject such as `IService&TComponent` never reached `TComponentIsaTypeSpecifyingExtension` and needed an inline `@var` to analyse. The tag ships with the framework source and needs no `phpstan.neon` entry. (belisoful)
ENH: Issue #120 - `TSoapServer::WsdlStyle` selects the style of a generated WSDL: `rpc` with SOAP encoding, as before and still the default, or `document` for WS-I Basic Profile 1.1, which prohibits it. `document` needs `pradosoft/prado-wsdlgenerator` 1.2, which also adds the `@soaptype` tag this issue asked for, and is refused where the installed generator predates it. (belisoful)
BUG: Issue #1267 - `TSoapService` could not configure a soap server from a PHP application configuration: `loadConfig()` stored a plain array where `createServer()` reads the `TAttributeCollection` the XML branch stores, and a `class` beside the properties was ignored. (belisoful)
BUG: `TUrlMappingPattern::getVerbs()` negation never matched. A pattern with `Verbs="!DELETE"` matched no request at all, since a verb had to be listed positively before a negation was consulted. A list of negated verbs alone now matches every method it does not exclude, a mixed list matches a method that is included and not excluded, and the comparison ignores case. Extracted as `TUrlMappingPattern::matchesVerb()`. (belisoful)
ENH: Issue #206 - `TUrlMappingPattern` matches the query string of a URL. `UrlMatchMode="Full"` matches the pattern, the text after its first `?` included, against the PATH_INFO and the query string; `query.<name>="<regex>"` constrains single GET variables whatever their order in the URL. `constructUrl()` appends the extra GET variables with `&` when the pattern already carries a query string. (belisoful)
ENH: TTabPanel inactive tabs gain a sheen for depth, with hover and `:active` shifting the gradient and the pressed tab dropping a pixel. The gradient runs lighter at the top under a light scheme and darker at the top under a dark one, so each colour stop is a `light-dark()` pair; a single translucent overlay would run the same way in both. The selected tab is unchanged and stays flat. (belisoful)
ENH: TSlider draws its handle in CSS instead of loading `TSliderHandleHorizontal.png` and `TSliderHandleVertical.png`, so the handle follows the declared color scheme; `light-dark()` resolves colors and not images, so a bitmap handle could not. The slider body and track borders, and the TAccordion normal header border, were declared without a color and so fell back to `currentColor`, drawing white under a dark scheme; each now carries an explicit color. The two handle PNGs are no longer referenced. (belisoful)
BUG: Playwright global setup cleared only the prado-demos published asset caches, so an edit to a framework CSS or JS source could be tested against the previously published copy under `tests/harness/<group>/assets/`; local results then disagreed with CI, which starts from a fresh checkout. Both asset roots are now cleared. (belisoful)
ENH: Prado controls render in both the Light and Dark color schemes. The shipped stylesheets for TTabPanel, TAccordion, TKeyboard, TSlider, TDatePicker, TColorPicker and the ActiveRecord Scaffold resolve their colors through `light-dark()` and the `Canvas`/`CanvasText` system colors, following the `color-scheme` an application or a single container declares; an application declaring none keeps the light scheme. Both schemes meet the WCAG 1.4.3 and 1.4.11 minimums, which moved several light colors that fell short. (belisoful)
ENH: Issue #777 - TRelativeTime subclasses TTime to show an elapsed time that updates live, such as "6 hours ago", and reveals the absolute time on click. `CultureInfo::getUnitPatterns()` and the CultureInfo Units supply the localized unit patterns. (belisoful)
...
ENH: TSafetyCover is a panel behind a protective cover that must be opened before the area beneath it accepts clicks, with a `ResetDelay` cooldown after the cover closes. (belisoful)
ENH: WebControl Accessibility Audit: ARIA roles and states, keyboard operation, and localized text across the ActiveControls, the in-place controls, TAccordion, TSlider, TTabPanel/TTabView, TBaseValidator, TValidationSummary, TColorPicker, TKeyboard, and TWizard. TTabView renders the caption of a JavaScript-switching tab as plain text on the `role="tab"` element instead of wrapping it in an `<a href="javascript://">` anchor; UPGRADE.md gives the CSS selector migration. (belisoful)
ENH: Issues #1249, #1250 - A handler stops an event early. `TEventParameter::Stopped` ends the handler loop of `TComponent::raiseEvent()`, and `TCallChain::Stopped` with `stopImmediatePropagation()` does the same for a dynamic event chain. (belisoful)
ENH: Issue #702 - TInPlaceDropDownList and TInPlaceListBox edit a value in place with a `<select>`. `TInPlaceControlTrait` and `TInPlaceListControlTrait` carry the properties common to the in-place controls; TInPlaceTextBox deprecates `AutoHideTextBox` for `AutoHideEditor` and `DisplayTextBox` for `DisplayEditor`. (belisoful)
ENH: Issue #636 - TFileValidator and TImageValidator validate an upload by count, size, media type, and image dimensions, on the client and on the server; TFileUpload adds `Accept` and `Capture`. TActiveFileUpload causes validation and takes a `ValidationGroup`, a behavior change documented in UPGRADE.md. (belisoful)
ENH: Issue #1242 - `composer.json` declares an `extra.branch-alias` of `4.4.x-dev` for `dev-master`, so an extension requires `pradosoft/prado: ^4.4` without naming the branch. (belisoful)
ENH: TWebTemplate and TActiveWebTemplate render the HTML5 `<template>` element, with `TShadowRootMode` and the shadow-root properties for a declarative shadow root, and optional tracking and postback persistence of stamped instances. (belisoful)
ENH: HTML5 controls: TDialog and TActiveDialog, TDetails, TSummary and TActiveDetails, TProgress and TActiveProgress, TMeter and TActiveMeter, TFigure and TFigureCaption, TTime for the `<time>` element, and TTableColumn and TTableColumnGroup with their collections for `<col>` and `<colgroup>`. (belisoful)
ENH: Prado IO (`framework/IO/`) core: `IResource` and `TResource` own a PHP resource handle, and `TStream` implements the PSR-7 `StreamInterface` with `TStreamFactory` for PSR-17. Ships TFileStream, TMemoryStream, TTempStream, TInputStream, TOutputStream, TStdInStream, TStdOutStream, and TStdErrStream, with `TByteOrder`, `TResourceType`, `TStreamMetadataKey`, and stream presets. `TOutputWriter` and `TStdOutWriter` are deprecated. (belisoful)
ENH: Prado IO streams: the utility sources and decorators TAppendStream, TCachingStream, TDroppingStream, TFnStream, TLazyOpenStream, TLimitStream, TMultipartStream, TNoSeekStream, TPumpStream, and the first-in first-out TBufferStream. TReservedSpaceStream and TFreeSpaceStream guard reserved byte ranges such as an EXIF private space. TBinaryStream and TBinaryStreamBehavior read and write binary numbers, TBitReader and TBitWriter read and write bits, TStreamFilter and TStreamCodecFilter register PHP stream filters, TStreamWrapper registers new protocols, and TStreamHelper copies, hashes, reads lines, and copies a byte range in place. (belisoful)
ENH: Prado IO compression: `ICompressor` and `IStreamCodec` standardize compression and decompression. TBuiltinCompressor wraps the native codecs (TGzipCompressor, TZlibCompressor, TDeflateCompressor, TBzip2Compressor, TZstdCompressor, TBrotliCompressor), TCliCompressor runs an external tool such as `xz`, and TCompression negotiates a content coding against an `Accept-Encoding` header. TInflateStream and TDeflateStream compress and decompress a stream as it is read or written. (belisoful)
ENH: Prado IO sockets: TSocketStream, TSocketServer, and TSocketAddress speak TCP, UDP, UNIX, and UDG with TLS and ALPN. TSocketReactor is a single-threaded `select()` event loop with timers that multiplexes listeners and connections. TSocketServerModule and TSocketServerAction run a socket server from the application or the CLI. (belisoful)
ENH: Prado IO processes: TProcess, TPipeStream, and TProcessStatus run a child process and communicate with it; TProcessFork, TLiveProcessFork, and TResultProcessFork fork the current process and return the child's result. (belisoful)
ENH: Prado IO HTTP: THttpClient wraps curl and fopen behind one interface, and TRestClient is the base for consumers of third-party REST APIs, turning typed method calls into requests, decoding JSON, and raising HTTP errors as exceptions. `TMediaType` maps a file extension to its media type, and THttpResponse gains the REST status codes it lacked. (belisoful)
CHG: Issue #1210 - TRpcService, TJsonRpcServer, TXmlRpcServer, and the RPC client moved to the `pradosoft/prado-rpc` repository. An application using RPC requires that package. (belisoful)
ENH: TClosureCronTask runs a closure as a cron task, and TProcessCronTask starts or ensures an external process on a schedule. (belisoful)
CHG: Application state, asset, cache, template, and authentication keys hash with sha1 in place of md5. Published asset paths and stored keys change; no application code changes. (belisoful)
ENH: Composer extensions register themselves. `TComposerReflection` reads package metadata and the Prado extras, `Prado::registerClassMap()` adds an extension's class map, and an application configuration takes `errorMessages` and `classMap` entries, so an extension such as `pradosoft/prado-sqlmap` no longer needs a module to install them. (belisoful)
ENH: Issues #836, #852 - Template and configuration values coerce to the type a setter declares. `TPropertyValue::applyProperty()`, `coerceForSetter()`, and `coerceToType()` back `TComponent::setSubProperty()`, `ensureArray()` parses an array literal by regular expression instead of `eval()`, and `ICoercible` lets a class construct itself during coercion. `TComponent::setSubProperties()` sets a group of properties in dependency order, and a `@` prefix addresses a named behavior through `getSubProperty()`/`setSubProperty()`. (belisoful)
ENH: TPropertyValue adds `ensureArrayOfType()`, `ensureNullIf()`, and `ensureEnumValue()`, with `ARRAY_`, `FILTER_`, `BOOL_`, `TYPE_`, and `CONFIG_ARRAY_OF_TYPE` constants; `ensureEnum()` is case insensitive, accepts a `\UnitEnum`, and takes extras such as `null` or `false`. `ensureEnum()` normalizes the constant name case, which UPGRADE.md notes. (belisoful)
ENH: Issue #831 - TAssetManager copies an asset atomically, directories included, and publishes a virtual asset from any `IPublishable` object. TDot is the first of them, an SVG status dot of a given size and color with computed main and highlight shading. (belisoful)
ENH: TIntegrityManager manages subresource integrity hashes for remote scripts, so a script is validated before the browser runs it. (belisoful)
ENH: Issue #1004 - Content Security Policy support. `THttpHeadersManager` is a module over THttpResponse that manages typed header objects: `THttpHeaderCsp` builds a policy from `TCspDirective`, and `THttpHeaderHsts`, `THttpHeaderReportingEndpoints`, `THttpHeaderContentType`, and `THttpHeaderContentDisposition` cover the rest. `TSecurityManager` generates the per-request nonce, TJavaScript and TJavaScriptAsset render every `<script>` with its nonce and integrity attributes, and `TCspReportingService` receives violation reports and raises `onViolation`. Adds `TMediaType`, `TContentDisposition`, and `THttpHeaderName`. (ctrlaltca, belisoful)
ENH: Caching refactor. `ICache::getIsAvailable()` reports whether a backend is usable, `TSerializingCache` serializes, encrypts, and encodes a value for a backend that stores strings, `TFileCache` and `TMemoryCache` bound their size through `ICacheSize` and `TCacheSizeTrait`, and TFileCache expires by file modification time and cleans up on a cron task. TCache, TAPCCache, TDbCache, TEtcdCache, TMemCache, and TRedisCache follow the Uniform Access Principle, and the cache dependencies are refactored alongside. (belisoful)
ENH: Issue #1098 - The Prado JavaScript is ECMAScript 2022 and is far less dependent on jQuery. An AJAX callback dispatches the `prado:ajaxComplete` event in place of the jQuery `onAjaxComplete` event; see UPGRADE.md. (belisoful)
ENH: TWebControl carries the HTML5 global attributes, `data-` attributes through `Dataset`, and the ARIA attributes, held in a `TWebAttributeCollection`. Adds `TWebInputMode`, `TEnterKeyHint`, and `TWebControlRenderTrait` for rendering a control's tags and contents. (belisoful)
ENH: `TUri` implements the PSR-7 `UriInterface` over the new `TResourceUri`, with `TUriScheme` and `TUriDefaultPort` covering HTTP, FILE, DATA, TCP, UDP, UNIX, UDG, SSL, and TLS. Scheme and host lower-case, the path, query, and fragment percent-encode, and `getPort()` returns `?int`; see UPGRADE.md. (belisoful)
ENH: `IModuleDependency` orders module initialization, so a module loads after the modules it depends on, and `TApplication::filterModuleDependencies()` adjusts the order application-wide. TUrlMapping, TDbParameterModule, TActiveRecordConfig, and TCacheHttpSession declare their dependencies, and `TModule` raises `dyPostInit`. (belisoful)
ENH: `TApplication::runModuleLifecycle()` adds a bootstrapped default manager to the application module list when no configured module takes its place, and `TApplication::StateFlag` holds the application state. (belisoful)
ENH: TEventLoggingBehavior logs every `on` and `dy` event of the object it is attached to. (belisoful)
ENH: TSerializableClosure serializes a closure, signed by TSecurityManager; adds the `laravel/serializable-closure` and `psr/clock` dependencies. (belisoful)
ENH: Reflection and behavior caching. `TComponentReflection` caches the `ReflectionClass`, `ReflectionMethod`, and `ReflectionProperty` of a type and is used framework-wide, `TComponent` caches the methods a behavior contributes, and `Prado::method_visible()` answers a public method without walking the backtrace. (belisoful)
ENH: `TEnumerable` refactor: `IEnumerable` is the general interface, `TConstantReflectionTrait` supplies the constants, and `TArrayIteratorTrait`, `TArrayCopyIteratorTrait`, and `TReflectionClassTrait` are reusable on their own. (belisoful)
ENH: PradoUnit is the unit test harness of the framework, available to an application or extension for its own tests: snapshot and restore of instance and static properties through the whole class hierarchy, private ancestor properties included; `invoke()` for a protected or private method; capture and restore of the process globals; `TTestApplication`, which runs without a configuration file, captures `exit()` and restores the `Prado` and `TComponent` static state; `setup*Connection()` factories for the seven supported databases; and the reusable `Harness/` classes and traits. (belisoful)
ENH: `TModuleConfigurationFileTrait` reads a module's external configuration file, shared by the user, permission, and other file-configured modules. (belisoful)
CHG: The deprecated `TWebColors` and `TDbCronModule` are removed; use `TWebColor` and `TDbCronManager`. (belisoful)
CHG: Issue #1131 - `TReCaptcha2::getResponse()` is renamed `getCaptchaResponse()`, as it overrode `TApplicationComponent::getResponse()`. (belisoful)
CHG: `TDatePicker::InputMode` is renamed `DateInputMode`, as `TWebControl::InputMode` now exists. (belisoful)
CHG: The functional test suite drops Selenium and runs on Playwright alone. (belisoful)
BUG: Issue #1196 - A TDatePicker popup opened above its input when the page body carried a margin. `getPositioningParent()` returned the body when no positioned ancestor existed, so the body's viewport offset was subtracted from an already viewport-relative rectangle. (ctrlaltca)
BUG: A click inside the TColorPicker popup closed it in Simple and Full mode, since the containment check only recognized the Basic mode container. (belisoful)
BUG: Database drivers: `TMssqlCommandBuilder::applyLimitOffset()` uses OFFSET/FETCH on SQL Server 2012 and later, `TOracleCommandBuilder` does the same when the table name is empty, `TOracleMetaData::getDefaultSchema()` loads the schema lazily with a fallback, and `TMysqlMetaData` honors table name case sensitivity. (belisoful)
BUG: A weak collection corrupted itself when a handler added or removed an element while the collection was scrubbing dead references. TWeakList, TWeakMap, and TWeakCallableCollection defer the change through `TWeakCollectionTrait` instead. (belisoful)
BUG: A property was assigned before its value was validated, so a rejected value left the object changed. TParameterizeBehavior, TTimeScheduler, TFileLogRoute, TLogRouter, TSoapService, TAssetManager, TUrlMapping, TThemeManager, and TTableStyle validate first. (belisoful)
BUG: TAssetManager audit fixes: a forced copy resolves the link path, and only the listed paths are exempt from publishing. `TBitHelper::floatToFpXX()` computed the value and `$IEEEConformance` incorrectly. (belisoful)
BUG: `TJavaScript::jsonEncode()` converts a string to UTF-8 before encoding, and guards a null value. (belisoful)
BUG: `TSimpleDateFormatter` returned the wrong date when the target month is shorter than the source day of month. (belisoful)
BUG: `TCallbackPageStateTracker::addStatesToTrack()` misspelled the `disabled` state, so it was never tracked. (belisoful)
