...
BUG: `TTemplate` rendered a dashed attribute name such as `Attributes.aria-label` with an underscore. Removes `TTemplate::attributeToMethodName()`. (belisoful)
BUG: The PHPStan guard extensions narrowed a virtual property under its lowercase-first spelling only, so `$obj->Title` inside a `canGetProperty()`, `canSetProperty()` or `hasMethod()` guard was reported as undefined. (belisoful)
ENH: `TComponent::isa()` narrows its subject for PHPStan through an `@phpstan-assert-if-true` tag, intersection types included. (belisoful)
ENH: Issue #120 - `TSoapServer::WsdlStyle` selects an `rpc` (default) or WS-I `document` style WSDL; `document` needs `pradosoft/prado-wsdlgenerator` 1.2, which adds `@soaptype`. (belisoful)
BUG: Issue #1267 - `TSoapService` could not configure a soap server from a PHP application configuration, and ignored a `class` beside its properties. (belisoful)
BUG: A `TUrlMappingPattern` verb negation such as `Verbs="!DELETE"` never matched. Verbs now compare without regard to case. Extracted as `TUrlMappingPattern::matchesVerb()`. (belisoful)
ENH: Issue #206 - `TUrlMappingPattern` matches the query string: `UrlMatchMode="Full"` includes the pattern's `?` part, and `query.<name>="<regex>"` constrains one GET variable in any order. (belisoful)
ENH: TTabPanel inactive tabs gain a gradient sheen with hover and pressed states in both color schemes. (belisoful)
ENH: TSlider draws its handle in CSS so it follows the color scheme; the handle PNGs are no longer used. TSlider and TAccordion borders no longer draw white under a dark scheme. (belisoful)
BUG: Playwright global setup also clears `tests/harness/<group>/assets/`, so a local run no longer tests stale published framework assets. (belisoful)
ENH: Prado controls render in the Light and Dark color schemes. The stylesheets of TTabPanel, TAccordion, TKeyboard, TSlider, TDatePicker, TColorPicker and the ActiveRecord Scaffold use `light-dark()` and system colors to follow the declared `color-scheme`, light by default, and meet WCAG 1.4.3 and 1.4.11 in both. (belisoful)
...
ENH: PradoUnit is the framework's unit test harness, available to applications and extensions: property snapshot and restore across the class hierarchy, `invoke()` for non-public methods, capture and restore of process globals, `TTestApplication` without a configuration file, database connection factories, and the reusable `Harness/` classes and traits. (belisoful)
