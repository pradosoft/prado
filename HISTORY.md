...
ENH: Prado controls render in both the Light and Dark color schemes. `accordion.css`, `tabpanel.css`, `keyboard.css`, `TSlider/TSlider.css`, `datepicker/default.css`, `colorpicker/default.css` and the ActiveRecord Scaffold `style.css` resolve their colors through `light-dark()` and the `Canvas`/`CanvasText` system colors, so controls follow the `color-scheme` the application declares, including a scheme declared on a single container. An application that declares no `color-scheme` keeps the light scheme. Both schemes meet the WCAG 1.4.3 and 1.4.11 minimums, which moved several long-standing light colors that fell short: the inactive and hovered tab text and the accent behind the tab borders, view border and focus outline (`TTabPanel`), the active header text and border (`TAccordion`), the key border and hovered key text (`TKeyboard`), and the panel borders (`TDatePicker`, `TColorPicker`). The `TKeyboard` pressed-key red also moved, in both schemes, so its white label clears 4.5:1. (belisoful)
...
ENH: PradoUnit is the unit test harness of the framework, and an application or an extension uses it for its own tests. `snapshot()`, `restore()`, `snapshotStatic()` and `restoreStatic()` read and write instance and static properties at every level of a class hierarchy, private ancestor properties included. `invoke()` calls a protected or private method. `captureInitialState()` and `restoreInitialState()` return the superglobals, working directory, default timezone, error reporting and include path to their baseline. `TTestApplication` runs a TApplication from any writable directory without a configuration file, captures `exit()` in place of ending the test runner, and restores the static state of `Prado` and `TComponent`. The `setup*Connection()` factories open a MySQL, PostgreSQL, SQLite, Firebird, MSSQL, Oracle or IBM connection, and `failFirstThenSkip()` fails the first test to meet a missing server, database or table and skips the rest. The `Harness/` classes and traits are reusable in a test case. (belisoful)
