# Web/Javascripts/INDEX.md - WEB_JAVASCRIPTS_INDEX.md

This file provides guidance to Agents when working with code in this repository.

## Purpose

Client-side JavaScript/CSS package registry and asset utilities for the Prado web layer. Defines the package dependency graph consumed by `TClientScriptManager` and provides PHP-side helpers for rendering script/style tags.

## PHP Classes

- **`TJavaScript`** — Static utility class. Methods:
...
- **`TJavaScriptAsset`** — Represents a published JS asset. Properties: `BaseUrl`, `Version`. Used by `TAssetManager` to generate cache-busted URLs.

- **`TJavaScriptLiteral`** — Wraps a raw JavaScript expression that should **not** be quoted when embedded in a JS context (e.g., a callback function reference). Distinguishes literal code from string values in `quoteJsLiteral()`.
...
- **`packages.php`** — Returns an associative array declaring all JS packages and their dependencies:
  ...
  // etc.
  ```
  `TClientScriptManager` uses this to resolve and publish packages in dependency order.
...
- **Never reference JS files directly** — always use the package name via `TClientScriptManager::registerPackage()`. The manager resolves dependencies, deduplicates, and handles publishing.
...
- **Asset versioning** — `TJavaScriptAsset::getVersion()` appends a version query parameter to bust browser caches on upgrade. Update it when making breaking JS changes.
