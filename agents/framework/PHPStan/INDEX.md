...
- PHPStan hands a method call to a `MethodTypeSpecifyingExtension` only when the subject type has **exactly one** object class name (`MethodCallHandler::specifyTypes()`). A subject with an intersection type (`IService&TComponent`) has two, so no method type-specifying extension ever sees it, and `isMethodSupported()` is never called. Narrowing for such subjects belongs in an `@phpstan-assert-if-true` tag on the method, which PHPStan evaluates for every subject type. `TComponentHasMethodTypeSpecifyingExtension`, `TComponentCanGetPropertyTypeSpecifyingExtension` and `TComponentCanSetPropertyTypeSpecifyingExtension` share the limitation. Their narrowing targets `HasMethodType` and `HasPropertyType`, which no PHPDoc tag expresses, so intersection subjects stay unnarrowed for those three.
