...
- **[TXmlDocument](./TXmlDocument.md)** — Extends [TXmlElement](./TXmlElement.md). Wraps `DOMDocument`. Load via `loadFromFile($path)` or `loadFromString($xml)`. Save via `saveToFile($path)` or `saveToString()` / `__toString()`. Properties: `Version`, `Encoding`. The `TagName` property is the root element name.

- **[TXmlElement](./TXmlElement.md)** — Represents a single XML element. Key properties and features:
  - `TagName` — element tag name
  - `Value` — text content (`nodeValue`; not CDATA or comments)
  - `Attributes` — `TMap` of name→value attribute pairs
  - `Elements` — [TXmlElementList](./TXmlElementList.md) of direct child elements
  - `Parent` — reference to parent [TXmlElement](./TXmlElement.md)
  - `xpath($expression)` — run XPath query; returns array of [TXmlElement](./TXmlElement.md).
...
- **[TXmlElementList](./TXmlElementList.md)** — `TList` subclass holding child [TXmlElement](./TXmlElement.md) objects. Maintains `Parent` references. Handles re-insertion correctly (removing from self before re-adding to avoid duplicates).
