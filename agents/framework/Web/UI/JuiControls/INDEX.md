# Web/UI/JuiControls/INDEX.md

### Directories
[framework](./INDEX.md) / [Web](./Web/INDEX.md) / [UI](./Web/UI/INDEX.md) / **`JuiControls/INDEX.md`**
...
- **[`IJuiOptions`](IJuiOptions.md)** — Interface: `getOptions()`, `getValidOptions()`, `getValidEvents()`, `getWidget()`, `getWidgetID()`.

- **[`TJuiControlAdapter`](TJuiControlAdapter.md)** — Extends `TActiveControlAdapter`. Publishes jQuery UI CSS/JS assets. Sets up `TJuiCallbackPageStateTracker` to watch `JuiOptions` changes during callbacks.

- **[`TJuiControlOptions`](TJuiControlOptions.md)** — Stores jQuery widget options in viewstate. `setOption($name, $value)` / `getOption($name)`. Validates option names against `getValidOptions()`. Options are serialised to JSON and passed to the widget's JavaScript initialiser.

- **[`TJuiCallbackPageStateTracker`](TJuiCallbackPageStateTracker.md)** — Tracks changes to `JuiOptions` during callbacks so only changed widget options are sent to the client.
