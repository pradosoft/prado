...
- **[`TActiveRecordAction`](TActiveRecordAction.md)** — Action `activerecord`. Generates Active Record PHP class skeletons from database tables.
...
- **[`THelpAction`](THelpAction.md)** — Action `help`. Prints usage information for all registered shell actions. Always available; lists action names, parameters, optional arguments, and descriptions.

- **[`TPhpShellAction`](TPhpShellAction.md)** — Action `shell`. Launches an interactive PHP REPL with the Prado application bootstrapped. Useful for ad-hoc debugging and scripting against live application state.

- **[`TWebServerAction`](TWebServerAction.md)** — Action `serve`. Starts the built-in PHP development web server for the application.
...
1. Create a class extending [`TShellAction`](../TShellAction.md) in this directory.
