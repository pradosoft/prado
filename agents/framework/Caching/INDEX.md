...
- **[`ICache`](ICache.md)** — Core cache contract. Methods: `get($id)`, `set($id, $value, $expire, $dependency)`, `add($id, $value, $expire, $dependency)`, `delete($id)`, `flush()`. All implementations must honour the `$expire` TTL (seconds; `0` = never) and invalidate entries when a dependency changes.

- **[`ICacheDependency`](ICacheDependency.md)** — Marker interface for dependency objects. Single method: `getHasChanged()` → bool. Must be serialisable (stored alongside the cached value).

## Base Classes

- **[`TCache`](TCache.md)** — Abstract base implementing [`ICache`](ICache.md) and `ArrayAccess`. Provides:
...
- **[`TMemCache`](TMemCache.md)** — Memcached distributed cache. Uses the `\Memcached` PHP extension. Supports multiple servers with `<server>` child elements in XML config (Host, Port, Weight). Property: `PersistentID` for cross-request connection pooling.

- **[`TRedisCache`](TRedisCache.md)** — Redis distributed cache. Uses the `\Redis` PHP extension. Properties: `Host`, `Port` (default `6379`), `Socket` (Unix socket), `Index` (DB 0–15). Uses PHP serialiser (`Redis::SERIALIZER_PHP`).
...
- **[`TDirectoryCacheDependency`](TDirectoryCacheDependency.md)** — Invalidates when any file in a directory changes. Properties: `RecursiveCheck`, `RecursiveLevel`. Override `validateFile()`/`validateDirectory()` for custom filtering.
- **[`TGlobalStateCacheDependency`](TGlobalStateCacheDependency.md)** — Invalidates when a named Prado global state variable changes.
- **[`TApplicationStateCacheDependency`](TApplicationStateCacheDependency.md)** — Invalidates unless the application is in `Performance` mode (useful for cache bypass in `Debug` mode).
- **[`TChainedCacheDependency`](TChainedCacheDependency.md)** — Composite dependency: invalidates if **any** child dependency reports changed (OR logic). Child list is a [`TCacheDependencyList`](TCacheDependencyList.md).
- **[`TCacheDependencyList`](TCacheDependencyList.md)** — Typed [`TList`](../Collections/TList.md) that enforces [`ICacheDependency`](ICacheDependency.md) on all items.
