...
test('ActiveCheckBoxListTestCase', async ({ page }) => {
  ...
  await h.url('active-controls/index.php?page=TActiveCheckBoxListTest');
  await h.assertSourceContains('TActiveCheckBoxList Test Case');

  await h.assertText(`${base}label1`, 'Label 1');

  await h.byId(`${base}button1`).click();
  await assertCheckBoxes([1, 2, 3]);
  ...
  await assertCheckBoxes([1, 4]);

  await h.byId(`${base}list1_c2`).click();
  await h.assertText(`${base}label1`, 'Selection: value 2, value 3, value 5');

  await h.byId(`${base}list1_c2`).click();
  await h.assertText(`${base}label1`, 'Selection: value 2, value 5');
});
