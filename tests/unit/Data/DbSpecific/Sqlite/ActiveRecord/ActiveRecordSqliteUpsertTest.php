...
require_once(__DIR__ . '/records/SqliteUpsertTestRecord.php');
...
/**
 * ActiveRecordSqliteUpsertTest — integration tests for {@see TActiveRecord::upsert()} on SQLite.
 *
 * Uses an in-memory SQLite database (created fresh per test class).
 * Skipped automatically when pdo_sqlite is unavailable.
 *
 ...
 */
class ActiveRecordSqliteUpsertTest extends PHPUnit\Framework\TestCase
{
	...
	protected function setUp(): void
	{
		...
		// Re-assert AR manager connection and reset the gateway on every setUp.
		// The gateway caches command builders by connection string ('sqlite::memory:').
		// A previous test class may have built and cached a builder for that string
		// against a different (now-closed) in-memory connection.  Resetting the
		// gateway via setGatewayClass() clears _commandBuilders so the next AR
		// operation creates a fresh builder against the current static::$conn.
		$manager = \Prado\Data\ActiveRecord\TActiveRecordManager::getInstance();
		$manager->setDbConnection(static::$conn);
		$manager->setGatewayClass(\Prado\Data\ActiveRecord\TActiveRecordManager::DEFAULT_GATEWAY_CLASS);
		static::$conn->createCommand('DELETE FROM upsert_test')->execute();
		static::$conn->createCommand('DELETE FROM sqlite_sequence WHERE name = \'upsert_test\'')->execute();
	}
	...
	{
		$record = new SqliteUpsertTestRecord();
	...
	{
		// int-keyed: ['score'] means "update score using the value from the record"
		static::$conn->createCommand(
			"INSERT INTO upsert_test (username, score) VALUES ('alice', 10)"
		)->execute();
	...
	{
		// string-keyed: ['score' => 99] means "update score to the literal value 99"
		static::$conn->createCommand(
			"INSERT INTO upsert_test (username, score) VALUES ('alice', 10)"
		)->execute();
	...
	{
		// Mixed: ['score', 'username' => 'alice_renamed'] — score from record, username explicit
		$original = new SqliteUpsertTestRecord();
		$original->username = 'alice';
		$original->score = 10;
		$original->upsert();
		...
		$renamed = SqliteUpsertTestRecord::finder()->find('id = ?', $originalId);
