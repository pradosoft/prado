...
/**
 * Live integration tests for TTableGateway — IBM DB2.
 *
 * Exercises insert, findByPk, find, findAll, findAllBySql, count, update,
 * deleteAll, deleteByPk, and TSqlCriteria against the real IBM DB2
 * prado_unitest database.
 *
 * Table schema (from initdb_ibm.sql):
 *   address (
 ...
 *     field2_date DATE,
 *     field3_dbl  DOUBLE,
 *     field4_int  INTEGER REFERENCES table1(id),
 ...
 *   )
 *
 * Note: field4_int references table1(id), which is pre-seeded with id=1.
 * Use field4_int=1 to satisfy the FK constraint.
 * Username is the PK (string). IBM DB2 uses double-quote identifiers.
 */
class TTableGatewayIbmIntegrationTest extends PHPUnit\Framework\TestCase
{
	...
	{
		if (self::$conn === null) {
			$this->markTestSkipped('IBM DB2 not available or address table missing.');
		}
	}
	...
	public function test_find_all_returns_inserted_rows(): void
	{
		$this->insertRecord1();
		$this->insertRecord2();
		$rows = self::$gateway->findAll("username LIKE 'tgw\\_%%' ESCAPE '\\'")->readAll();
	...
	public function test_find_all_with_criteria_limit(): void
	{
		$this->insertRecord1();
		$this->insertRecord2();
		// DB2 uses FETCH FIRST N ROWS ONLY; TSqlCriteria Limit must apply.
		$criteria = new TSqlCriteria("username LIKE 'tgw\\_%%' ESCAPE '\\'");
		$criteria->Limit = 1;
		$rows = self::$gateway->findAll($criteria)->readAll();
