...
/**
 * ActiveRecordMysqlUpsertTest — integration tests for {@see TActiveRecord::upsert()} on MySQL.
 *
 ...
 *   - resolveUpdateData modes: column-name list (int-keyed), explicit override (string-keyed), mixed
 *
 * Requires: prado_unitest MySQL database with the `upsert_test` table
 *   (see tests/initdb_mysql.sql).
 */
class ActiveRecordMysqlUpsertTest extends PHPUnit\Framework\TestCase
{
	...
	{
		$original = new MysqlUpsertTestRecord();
		$original->username = 'alice';
		$original->score = 10;
		$original->upsert();

		$update = new MysqlUpsertTestRecord();
	...
	public function test_upsert_explicit_value_updateData_overrides_record(): void
	{
	...
	public function test_upsert_mixed_updateData_handles_both_modes(): void
	{
		...
		// username was explicitly renamed; score was taken from the record
		$renamed = MysqlUpsertTestRecord::finder()->find('id = ?', $originalId);
	...
	public function test_upsert_does_not_affect_other_rows(): void
	{
		...
		$bob = MysqlUpsertTestRecord::finder()->find('username = ?', 'bob');
	...
	public function test_upsert_string_pk_new_record_returns_truthy(): void
	{
		...
		// cleanup
		MysqlUserRecord::finder()->findByPk('upsertTestUser')?->delete();
	}
	...
	public function test_upsert_string_pk_conflict_updates_row(): void
	{
		...
		$adminOriginal = MysqlUserRecord::finder()->findByPk('admin');
