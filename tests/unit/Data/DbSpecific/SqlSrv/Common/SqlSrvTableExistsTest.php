...
class SqlSrvTableExistsTest extends PHPUnit\Framework\TestCase
{
	...
	public function test_sqlsrv_getTableExists_returns_true_for_existing_table(): void
	{
		$gateway = new TTableGateway('upsert_test', static::$conn);
		$this->assertTrue($gateway->getTableExists());
	}

	public function test_sqlsrv_getTableExists_returns_true_for_newly_created_table(): void
	{
		static::$conn->createCommand(
	...
	public function test_sqlsrv_getTableExists_returns_false_after_table_is_dropped(): void
	{
		static::$conn->createCommand(
			'CREATE TABLE dbo.' . self::TEMP_TABLE . ' (id INT NOT NULL IDENTITY(1,1) PRIMARY KEY)'
		)->execute();
