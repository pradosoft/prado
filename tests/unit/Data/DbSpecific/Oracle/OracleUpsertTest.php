 ...
 *
 * Oracle upsert uses MERGE INTO ... WHEN MATCHED THEN UPDATE SET ...
 * WHEN NOT MATCHED THEN INSERT with USING (SELECT ... FROM DUAL).
 * An active transaction is required; TDbException is thrown otherwise.
 *
 * Oracle MERGE uses bare t / s aliases (useAsAlias=false, no AS keyword).
 ...
 *   upsert_test(username VARCHAR2(100) PK, score NUMBER(10) DEFAULT 0 NOT NULL)
 */
...
class OracleUpsertTest extends PHPUnit\Framework\TestCase
{
	...
	public function test_sql_update_set_contains_non_pk_columns(): void
	{
		...
		// username is PK, not in UPDATE SET target
