 ...
 * without one and the correct MERGE SQL generated/executed within one.
 *
 * upsert_test table uses username as natural-key PK (no identity) so MERGE ON
...
class MssqlInsertOrIgnoreTest extends PHPUnit\Framework\TestCase
{
	...
	public function test_sql_merge_on_clause_uses_pk_column(): void
	{
		...
		$gw->insertOrIgnore(['username' => 'alice', 'score' => 10]);
		$txn->rollback();
		// upsert_test PK is username → ON clause references [username]
